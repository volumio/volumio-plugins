...
ControllerDsp4Volumio.prototype.getUIConfig = function () {
  ...
  self.commandRouter.i18nJson(__dirname + '/i18n/strings_' + lang_code + '.json',
    ...
    .then(function (uiconf) {
      ...
      let filtersources = "/data/INTERNAL/Dsp4Volumiofilters/filter-sources";
      let items;
      let allfilter;
      let oformat;
      let filetoconvertl;
      let bkpath = "/data/INTERNAL/Dsp4Volumiofilters/target-curves";
      let tc;
...
ControllerDsp4Volumio.prototype.setAdditionalConf = function (type, controller, data) {
  const self = this;
...
ControllerDsp4Volumio.prototype.hwinfo = function () {
  ...
  exec('/data/plugins/audio_interface/Dsp4Volumio/hw_params hw:' + output_device + ' >/data/configuration/audio_interface/dsp4Volumio/hwinfo.json ', {
    ...
    if (error) {
      ...
      fs.readFile('/data/configuration/audio_interface/Dsp4Volumio/hwinfo.json', 'utf8', function (err, hwinfo) {
        if (err) {
...
ControllerDsp4Volumio.prototype.saveVolumioconfig = function () {
  const self = this;
...
ControllerDsp4Volumio.prototype.restoreVolumioconfig = function () {
  const self = this;
  return new Promise(function (resolve, reject) {
    setTimeout(function () {
...
ControllerDsp4Volumio.prototype.autoconfig = function () {
  ...
  self.saveVolumioconfig()
    ...
    .then(self.rebuildcamilladspRestartDaemon()) //no sure to keep it..
...
ControllerDsp4Volumio.prototype.outputDeviceCallback = function () {
  const self = this;
  let defer = libQ.defer();
  setTimeout(function () {
...
ControllerDsp4Volumio.prototype.setLoopbackoutput = function () {
  const self = this;
  let defer = libQ.defer();
  let outputp
  ...
  let stri = {
    "output_device": {
      "value": "Loopback",
      "label": (outputp + " through Dsp4Volumio"),
    },
    "mixer_type": {
...
ControllerDsp4Volumio.prototype.setVolumeParameters = function () {
  const self = this;
  ...
  setTimeout(function () {
    ...
    const settings = {
      ...
      // need to set the device name of the original device Dsp4Volumio is controlling
...
//----------- we save the alsa configuration for future needs here, note we prepend alsa_ to avoid confusion with other Dsp4Volumio settings
ControllerDsp4Volumio.prototype.saveHardwareAudioParameters = function () {
  const self = this;
  let defer = libQ.defer();
  let conf;
  //volumestart
...
ControllerDsp4Volumio.prototype.restoresettingwhendisabling = function () {
  const self = this;
...
ControllerDsp4Volumio.prototype.createCamilladspfile = function () {
  const self = this;
  ...
  try {
    fs.readFile(__dirname + "/camilladsp.conf.yml", 'utf8', function (err, data) {
      ...
      let filter_path = "/data/INTERNAL/Dsp4Volumiofilters/";
      let vobaf_filter_path = "/data/INTERNAL/Dsp4Volumiofilters/VoBAFfilters";
      ...
      if (self.config.get('vatt'))
        if (self.config.get('filter_format') == "text") {
          f_ext = ".txt";
        } else if (self.config.get('filter_format') == "FLOAT32LE") {
          f_ext = ".pcm";
        } else if (self.config.get('filter_format') == "FLOAT64LE") {
          f_ext = ".dbl";
        } else if ((self.config.get('filter_format') == "S16LE") || (self.config.get('filter_format') == "S24LE") || (self.config.get('filter_format') == "S32LE")) {
          f_ext = ".wav";
        }
      ...
      if (((self.config.get('filter_format') == "S32LE") || (self.config.get('filter_format') == "S24LE") || (self.config.get('filter_format') == "S16LE") || ((self.config.get('filter_format') == "FLOAT64LE") && (f_ext = ".wav"))) && (noldirac != "Dirac pulse")) {
        skipfl = "skip:44;"
      } else skipfl = "";
...
ControllerDsp4Volumio.prototype.dfiltertype = function (data) {
  const self = this;
  let extset;
  let filtername = self.config.get('leftfilter');
  let filterpath = '/data/INTERNAL/Dsp4Volumiofilters/';
  ...
  if (filext == 'pcm') {
    ...
    self.logger.info('------->filter ' + filext + ' FLOAT32LE');
  }
  else if (filext == 'wav') {
    try {
      wavFileInfo.infoByFilename(filterpath + filtername, function (err, info) {
        ...
        if (wavetype == '16') {
          auto_filter_format = 'S16LE';
          self.config.set('filter_format', auto_filter_format);
          self.logger.info('------->filter ' + filext + auto_filter_format + wavetype);
        }
        else if (wavetype == '24') {
          auto_filter_format = 'S24LE';
          self.config.set('filter_format', auto_filter_format);
          self.logger.info('------->filter ' + filext + ' S24LE ' + wavetype);
        }
        else if (wavetype == '32') {
          auto_filter_format = 'S32LE';
          self.config.set('filter_format', auto_filter_format);
          self.logger.info('------>filter ' + filext + ' S32LE ' + wavetype);
        }
        else if (wavetype == '64') {
          auto_filter_format = 'FLOAT64LE';
          self.config.set('filter_format', auto_filter_format);
          self.logger.info('------>filter ' + filext + ' FLOAT64LE ' + wavetype);
        }
      })
    }
    catch (err) {
    ...
    auto_filter_format = 'FLOAT64LE';
    self.config.set('filter_format', auto_filter_format);
    self.logger.info('--------->filter ' + filext + ' FLOAT64LE');
  }
    ...
    let modalData = {
      ...
      message: "<br>Your file is not supported. Please choose one of the supported format :<br>Supported type:<br><ul><li>text- 32/64 bits floats line (.txt) in rephase</li><li>S16LE- 16 bits LPCM mono (.wav) in rePhase</li><li>S24LE- 24 bits LPCM mono (.wav) in rePhase</li><li>S24LE- 32 bits LPCM mono (.wav) in rePhase</li><li>FLOATLE- 32 bits floating point (.pcm)</li><li>FLOAT64LE- 64 bits IEEE-754 (.dbl) in rephase</li></ul><br>",
      size: 'lg',
      buttons: [{
        name: 'Close',
        class: 'btn btn-warning'
...
ControllerDsp4Volumio.prototype.saveDsp4VolumioAccount2 = function (data) {
  const self = this;
  let output_device
  let input_device = "Loopback,1";
  ...
  if (self.config.get('leftfilter').split('.').pop().toString() != self.config.get('rightfilter').split('.').pop().toString()) {
    ...
    let cp3 = exec('/bin/cp /data/configurDsp4Volumio/config.json-save /data/configurDsp4Volumio/config.json');
    self.logger.info('/data/configurDsp4Volumio/config.json restored!');
      ...
      var responseData = {
        ...
        buttons: [
          ...
          },
          {
            ...
            class: 'btn btn-info',
            emit: 'callMethod',
            payload: { 'endpoint':'Dsp4Volumio', 'method': 'testclipping' }
          }
        ]
      }
...
ControllerDsp4Volumio.prototype.areSwapFilters = function () {
  ...
  setTimeout(function () {
    var respconfig = self.commandRouter.getUIConfigOnPlugin('audio_interface', 'brutefirDsp4Volumio', {});
    respconfig.then(function (config) {
      self.commandRouter.broadcastMessage('pushUiConfig', config);
...
ControllerDsp4Volumio.prototype.SwapFilters = function () {
  ...
  if (rsetUsedOfFilters == '1') {
    ...
    camilladspcmd = ('cfc "l_out" "' + "l_out" + '" ;cfc "r_out" "' + "r_out" + '"');

    self.config.set('displayednameofset', "Set used is _1");

  };

  self.sendCommandTocamilladsp(camilladspcmd);
...
ControllerDsp4Volumio.prototype.installtools = function (data) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      let cp3 = execSync('/usr/bin/wget -P /tmp https://github.com/balbuze/volumio-plugins/raw/master/plDsp4Volumio3/tools/tools.tar.xz');
      let cp4 = execSync('/bin/mkdir /data/Dsp4Volumio/tools');
      let cp5 = execSync('tar -xvf /tmp/tools.tar.xz -C /data/Dsp4Volumio/tools');
...
ControllerDsp4Volumio.prototype.removetools = function (data) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      let cp6 = execSync('/bin/rm -Rf /data/Dsp4Volumio/tools');
    } catch (err) {
...
ControllerDsp4Volumio.prototype.playFile = function (track, ititle, imessage) {
  const self = this;
  self.commandRouter.pushConsoleMessage('[' + Date.now() + '] ' + 'ControllerDsp4Volumio::clearAddPlayTrack');
...
ControllerDsp4Volumio.prototype.playleftsweepfile = function () {
  const self = this;
  ...
  let track = '/data/Dsp4Volumio/tools/V5.19_MeasSweep_44k_L_refL.WAV';
  self.playFile(track, ititle, imessage);
};

//here we play left sweep when button is pressed for REW >= 5.20
ControllerDsp4Volumio.prototype.playleftsweepfile520 = function () {
  const self = this;
  let ititle = 'Rew 5.20 - Sweep tools sample rate';
  let imessage = 'Please set sample rate to 44.1kHz and save the configuration in order to use this file';
  let track = '/data/Dsp4Volumio/tools/V5.20_MeasSweep_44k_L_refL.WAV';
  self.playFile(track, ititle, imessage);
};

//here we play right sweep when button is pressed for REW <= 5.19
ControllerDsp4Volumio.prototype.playrightsweepfile = function () {
  const self = this;
  let ititle = 'Rew 5.19 - Sweep tools sample rate';
  let imessage = 'Please set sample rate to 44.1kHz and save the configuration in order to use this file';
  let track = '/data/Dsp4Volumio/tools/V5.19_MeasSweep_44k_R_refL.WAV';
  self.playFile(track, ititle, imessage);
};

//here we play right sweep when button is pressed foor REW >= 5.20
ControllerDsp4Volumio.prototype.playlrightsweepfile520 = function () {
  const self = this;
  let ititle = 'Rew 5.20 - Sweep tools sample rate';
  let imessage = 'Please set sample rate to 44.1kHz and save the configuration in order to use this file';
  let track = '/data/Dsp4Volumio/tools/V5.20_MeasSweep_44k_R_refL.WAV';
  self.playFile(track, ititle, imessage);
};
...
ControllerDsp4Volumio.prototype.playleftpinkfile = function () {
  const self = this;
  let ititle = 'Pink tools sample rate';
  let imessage = 'Please set sample rate to 44.1kHz and save the configuration in order to use this file';
  let track = '/data/Dsp4Volumio/tools/PinkNoise_44k_L.WAV';
  self.playFile(track, ititle, imessage);
};

//here we play right pink noise channel when button is pressed
ControllerDsp4Volumio.prototype.playrightpinkfile = function () {
  const self = this;
  let ititle = 'Pink tools sample rate';
  let imessage = 'Please set sample rate to 44.1kHz and save the configuration in order to use this file';
  let track = '/data/Dsp4Volumio/tools/PinkNoise_44k_R.WAV';
  self.playFile(track, ititle, imessage);
};

//here we play both pink noise channels when button is pressed
ControllerDsp4Volumio.prototype.playbothpinkfile = function () {
  const self = this;
  let ititle = 'Pink tools sample rate';
  let imessage = 'Please set sample rate to 44.1kHz and save the configuration in order to use this file';
  let track = '/data/Dsp4Volumio/tools/PinkNoise_44k_BOTH.WAV';
  self.playFile(track, ititle, imessage);
};

//here we stop aplay
ControllerDsp4Volumio.prototype.stopaplay = function (track) {
  const self = this;
  try {
    exec('/usr/bin/killall aplay');
  } catch (e) {
...
ControllerDsp4Volumio.prototype.convert = function (data) {
  const self = this;
  //let defer = libQ.defer();
  let inpath = "/data/INTERNAL/Dsp4Volumiofilters/filter-sources/";
  let drcconfig = self.config.get('drcconfig');
  let outpath = "/data/INTERNAL/Dsp4Volumiofilters/";
  ...
  if (infile != 'choose a file') {
    ...
    if (tc != 'choose a file') {
      ...
      if ((outsample == 44100) || (outsample == 48000) || (outsample == 88200) || (outsample == 96000)) {
        ...
        let tcpath = "/data/INTERNAL/Dsp4Volumiofilters/target-curves/"
