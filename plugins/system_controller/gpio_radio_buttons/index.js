...
gpioRadioButtons.prototype.createTriggers = function() {
	...
	for (var index = 1; index <= buttonCount; index++) {
		...
		if (isEnabled === true) {
			...
			var j = new Gpio(gpioPin,'in','rising', {debounceTimeout: 250});
			j.watch(self.listener.bind(self, {stationName: stationName, stationUri: stationUri}));
...
gpioRadioButtons.prototype.listener = function(station, err, value) {
   ...
   this.logger.info('GPI Radio Buttons: Changing station to ' + station.title);
