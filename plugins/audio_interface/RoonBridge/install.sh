...
TMPDIR=`mktemp -d`
INSTALL_DIR="/data/plugins/audio_interface/$PACKAGE_NAME"
echo $1
...
User=root
Environment=ROON_DATAROOT=/data/configuration/audio_interface/$PACKAGE_NAME
Environment=ROON_ID_DIR=/data/configuration/audio_interface/PACKAGE_NAME
Environment=DAEMON_PIDFILE=/tmp/${PACKAGE_NAME_LOWER}.pid
Environment=DAEMON_LOGFILE=/tmp/${PACKAGE_NAME_LOWER}.log
ExecStart=/data/plugins/audio_interface/$PACKAGE_NAME/$PACKAGE_NAME/start.sh
Restart=always
