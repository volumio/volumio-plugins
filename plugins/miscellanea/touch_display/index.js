...
var device, xorgSocket, autoBrTimer, setScrToTimer1, setScrToTimer2, setScrToTimer3;
    ...
    .then(function () {
      ...
      exec('/bin/systemctl status volumio-kiosk.service', { uid: 1000, gid: 1000 }, function (error, stdout, stderr) {
        if (error !== null) {
          ...
          xorgSocket = '/tmp/.X11-unix/X' + stdout.slice(0, stdout.indexOf(' '));
          self.logger.info('Using Xorg socket ' + xorgSocket);
...
TouchDisplay.prototype.closeModals = function () {
  const self = this;
