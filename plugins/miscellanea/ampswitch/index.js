...
AmpSwitchController.prototype.getUIConfig = function () {
	...
	self.logger.ASdebug('Off pulse width: ' +
		self.config.get('off_pulse_width'))
	...
	.then(function (uiconf) {
		...
		uiconf.sections[0].content[0].value.label =
			self.config.get('port').toString();
		uiconf.sections[0].content[1].value =
			self.config.get('inverted');
		uiconf.sections[0].content[2].value =
			self.config.get('delay');
		uiconf.sections[0].content[3].value =
			self.config.get('latched');
		uiconf.sections[0].content[4].value =
			self.config.get('on_pulse_width');
		uiconf.sections[0].content[5].value =
			self.config.get('off_pulse_width');
...
AmpSwitchController.prototype.saveOptions = function (data) {
	...
	self.logger.ASdebug('Saving Settings: Port: ' +
		data['port_setting']['value']);
	self.logger.ASdebug('Saving Settings: Inverted: ' +
		data['inverted_setting']);
	self.logger.ASdebug('Saving Settings: Delay: ' +
		data['delay_setting']);
	self.logger.ASdebug('Saving Settings: Latched: ' +
		data['latched_setting']);
	self.logger.ASdebug('Saving Settings: On Pulse width: ' +
		data['on_pulse_width_setting']);
	self.logger.ASdebug('Saving Settings: Off Pulse width: ' +
		data['off_pulse_width_setting']);
	...
	if (successful) {
		...
		self.commandRouter.pushToastMessage('success',
			'Amplifier Switch Settings', 'Saved');
	} else {
...
AmpSwitchController.prototype.parseStatus = function (state) {
	...
	if (state.status == 'play' && state.status != status) {
		status = state.status;
		self.config.get('latched') ?
		self.pulse(self.config.get('on_pulse_width')) :
		self.on();
		...
		self.OffTimerID = setTimeout(function () {
				status = state.status;
				self.config.get('latched') ?
				self.pulse(self.config.get('off_pulse_width')) :
				self.off();
			}, delay);
	}
};
