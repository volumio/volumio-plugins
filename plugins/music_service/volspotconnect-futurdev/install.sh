...
then
	...
	wget https://github.com/balbuze/volumio-plugins/raw/master/plugins/music_service/volspotconnect-futurdev/arch/armhf/librespot-armhf.tar.xz
	if [ $? -eq 0 ]
		then
...
then
	cd $libpath
        echo "Cpu is $cpu, downloading required package."
	wget https://github.com/balbuze/volumio-plugins/raw/master/plugins/music_service/volspotconnect-futurdev/arch/x86/librespot-x86.tar.xz
	if [ $? -eq 0 ]
		then
			echo "Extracting data"
