...
playMusicController.prototype.onStart = function() {
  ...
  service.mpdPlugin = service.commandRouter.pluginManager.getPlugin(
    "music_service",
    "mpd"
  );
  ...
  if (masterToken && androidId && masterToken.length > 0) {
    defer.resolve(getTokenFromPlayMusic(service, masterToken, androidId));
  } else {
    var messageInfo = {
      messageType: "info",
      messageTitle: "To Enjoy Play Music",
      messageDisc: "Please Sign into Play Music to enjoy Google Play Music."
    };
...
function getTokenFromPlayMusic(service, masterToken, androidId) {
  ...
  service.playMusic.init(googleAuthData, function(error) {
    if (error) {
      ...
      var messageInfo = {
        messageType: "error",
        messageTitle: "Failed to get token to access Google Play Music",
        messageDisc: error.Error
      };
      service.showNotification(messageInfo);
    ...
    createExpressServer(service.playMusic).then(function(localProxyUrlData) {
      ...
      var messageInfo = {
        ...
        messageDisc: "You have successfully signed in the Play Music Account."
      };
      service.showNotification(messageInfo);
      defer.resolve();
    });
...
playMusicController.prototype.saveGoogleAccount = function(credential) {
  ...
  service.playMusic.login({ email: email, password: password }, function(
    err,
    authTokenData
  ) {
    if (err) {
      ...
      var messageInfo = {
        messageType: "error",
        messageTitle: "Google Login Failed",
        messageDisc: ""
      };
      service.showNotification(messageInfo);
      return defer.reject(new Error());
...
playMusicController.prototype.logoutGoogleAccount = function() {
  var service = this;
  service.config.set("masterToken", "");
  service.config.set("androidId", "");
  service.onStart();
  return libQ.resolve();
};
...
playMusicController.prototype.getUIConfig = function() {
  ...
  self.commandRouter
    ...
    .then(function(uiconf) {
      if (self.config.get("masterToken")) {
        defer.resolve(uiconf.logout);
      } else {
        defer.resolve(uiconf.login);
      }
    })
    .fail(function() {
...
playMusicController.prototype.handleBrowseUri = function(curUri) {
  ...
  if (curUri == "googleplaymusic") {
    ...
    if (!masterToken || masterToken.length == 0) {
      var messageObj = {
        messageType: "info",
        messageTitle: "Please Login First",
        messageDisc:
          "After login please click again on Play Music Sign in Browse section."
      };
    ...
    listItemsToRender =
      curUri == "googleplaymusic/playlists"
        ? getPlaylists(self)
        : getSongsInPlaylist(self, info);
    ...
    var stationInfo = {
      curUri: curUri,
      stationId: curUri.split("/").pop(),
      addToQueue: false
    };
    listItemsToRender =
      curUri == "googleplaymusic/stations"
        ? getStations(self)
        : getTracksInStation(self, stationInfo);
...
function getPlaylistTracksForQueue(service, uri) {
  ...
  var playlistId = isSharedPlaylist
    ? uri.split(":").pop()
    : uri.split("/").pop();
  var info = {
    curUri: uri,
...
playMusicController.prototype.renderAlbumTracks = function(curUri) {
  ...
  var response = JSON.parse(
    JSON.stringify(PLAY_MUSIC_CONSTANTS.FOLDER_OBJECT_STRUCTRE)
  );
  ...
  getAlbumTracks(self, albumId)
    ...
    .fail(function(error) {
      var messageInfo = {
        messageType: "error",
        messageTitle: "Error Getting Album Tracks",
        messageDisc: error
      };
...
playMusicController.prototype.getArtistData = function(curUri) {
  ...
  getArtistData(self, artistId)
    ...
    .fail(function(error) {
      var messageInfo = {
        messageType: "error",
        messageTitle: "Error Getting Artist data",
        messageDisc: error
      };
      self.showNotification(messageInfo);
      console.error(
        "Error getting artist data from Google Play Music Server.",
        error
      );
      defer.reject(error);
    });
...
playMusicController.prototype.clearAddPlayTrack = function(track) {
  ...
  try {
    self
      .playStreamUrl(localProxyUrl)
      .then(function() {
        defer.resolve();
      })
      .fail(function(error) {
        console.error(
          "Failed to Play song from Stream url " + localProxyUrl,
          error
        );
        defer.reject(error);
      });
    ...
    var messageInfo = {
      messageType: "error",
      messageTitle: "Error Playing Song",
      messageDisc: error
    };
    self.showNotification(messageInfo);
    defer.reject(error);
...
playMusicController.prototype.playStreamUrl = function(streamUrl) {
  ...
  return self.mpdPlugin
    .sendMpdCommand("stop", []) // sending command to stop current playing song.
    .then(function() {
    ...
    .then(function() {
      self.mpdPlugin.clientMpd.on("system", function(status) {
        ...
        self.mpdPlugin.getState().then(function(state) {
          ...
          return self.commandRouter.stateMachine.syncState(
            state,
            "googleplaymusic"
          );
        });
      ...
      return self.mpdPlugin.sendMpdCommand("play", []).then(function() {
        ...
        return self.mpdPlugin.getState().then(function(state) {
          ...
          self.commandRouter.pushConsoleMessage(
            "googleplaymusic: " + JSON.stringify(state)
          );
...
playMusicController.prototype.prefetch = function(trackBlock) {
  ...
  defer.resolve(
    self.mpdPlugin
      .sendMpdCommand('add "' + localProxyUrl + '"', [])
      .then(function() {
...
function getPlaylists(service) {
  ...
  service.playMusic.getPlayLists(function(error, apiResponse) {
    if (error) {
      ...
      return defer.reject(
        "unsuccessfull from getting music playlist from google server" + error
      );
    }
    var playlistsFormatedData = JSON.parse(
      JSON.stringify(PLAY_MUSIC_CONSTANTS.FOLDER_OBJECT_STRUCTRE)
    );
    playlistsFormatedData.navigation.prev.uri = "googleplaymusic";
    if (apiResponse.data) {
      playlistsFormatedData.navigation.lists[0].items = apiResponse.data.items.reduce(
        function(acc, playlist) {
      ...
      prefetchAllPlaylistTracks(service) // prefetching all playlist songs
        .then(function() {
          defer.resolve(playlistsFormatedData);
        }) // after fetching all tracks from playlist we are sending the array of playlists.
        .fail(function(error) {
          defer.reject(error);
        });
    } else {
      var messageInfo = {
        messageType: "error",
        messageTitle: "Error Getting Playlist",
        messageDisc:
          "No playlist found, perhaps you do not have monthly subscription."
      };
      service.showNotification(messageInfo);
      defer.reject(
        "No song found, Perhaps you do not have monthly subcription."
      );
    }
  });
  return defer.promise;
...
function getSongsInPlaylist(service, playlist) {
  ...
  if (playlist.addToQueue) {
    ...
    response.navigation.lists[0].items = retrievSongsOfAPlaylist(
      service.tracks,
      playListId
    );
  }
...
function getSongsOfASharedPlaylist(service, playlist) {
  ...
  service.playMusic.getSharedPlayListEntries(options, function(
    error,
    responseData
  ) {
    if (error) {
      ...
      var sharedPlaylistSongs = tracks.reduce(function(acc, track) {
        ...
        var volumioFormatSong = getVolumioFormatOfSong(
          track.trackId,
          trackData
        );
      ...
      var response = JSON.parse(
        JSON.stringify(PLAY_MUSIC_CONSTANTS.FOLDER_OBJECT_STRUCTRE)
      ); // else we need to send data in a specific format, so can volumio player can handle it.
      response.navigation.prev = playlist.curUri;
...
function getStations(service) {
  ...
  service.playMusic.getStations(function(error, apiResponse) {
    ...
    var formatedListOfStation = JSON.parse(
      JSON.stringify(PLAY_MUSIC_CONSTANTS.FOLDER_OBJECT_STRUCTRE)
    );
...
function getTracksInStation(service, stationInfo) {
  ...
  service.playMusic.getStationTracks(stationId, 25, function(
    error,
    apiResponse
  ) {
    if (error) {
    ...
    if (apiResponse.data.stations[0].tracks) {
      var stationTracks = apiResponse.data.stations[0].tracks.reduce(function(
        acc,
        track
      ) {
      ...
      if (stationInfo.addToQueue) response = stationTracks;
      // if the api request was to add songs in the queue then we just need to return array of station songs.
      else response.navigation.lists[0].items = stationTracks; // else we need to return in a specific format
      defer.resolve(response);
    } else {
      var messageInfo = {
        messageType: "error",
        messageTitle: "Error Getting Station Songs",
        messageDisc:
          "No song found, Perhaps you do not have montly subscription."
      };
      service.showNotification(messageInfo);
      defer.reject(
        "No song found, Perhaps you do not have monthly subcription."
...
function getAlbumTracks(service, albumId) {
  ...
  service.playMusic.getAlbum(albumId, true, function(error, albumResponse) {
    ...
    var volumioFormatSongList = albumResponse.tracks.reduce(function(
      acc,
      track
    ) {
...
function getTrackInfo(service, uri) {
  ...
  if (uri.includes("station")) {
    ...
    if (!trackResult) {
      var messageInfo = {
        messageType: "error",
        messageTitle: "Lost Tracks Data",
        messageDisc:
          "Please click on the playlist/album again, or search again."
      };
      service.showNotification(messageInfo);
      return {};
    }
  ...
  if (!trackInfo) {
    var messageObj = {
      messageType: "error",
      messageTitle: "Lost Tracks Data",
      messageDisc: "Please click on the playlist/album again, or search again."
    };
      ...
      https.get(streamUrl, function(streamUrlResponse) {
        ...
        if (streamUrlResponse.statusCode === 200) {
          wrapperID3(request.query.id, function(sts, data) {
            ...
            streamUrlResponse.on("data", function(chunk) {
              response.write(chunk);
            });
            streamUrlResponse.on("end", function() {
              response.end();
            });
          });
        } else response.end();
...
function createExpressServer(playMusicInstance) {
  ...
  server.setTimeout(server_timeout, function(msg) {
    console.log("Timeout for proxy server:", msg);
  });
  return defer.promise;
}
...
playMusicController.prototype.search = function(query) {
  var service = this;
  var defer = libQ.defer();
  searchQuery(service, query.value)
    .then(function(categoryData) {
      defer.resolve(categoryData);
    })
    .fail(function(error) {
      console.error(
        "Error getting song based on search query from Google Play Music Server.",
        error
      );
      defer.reject(error);
    });
...
function searchQuery(service, queryString) {
  var defer = libQ.defer();
  service.playMusic.search(queryString, 10, function(error, responseSongs) {
    // the second parameter is for returned songs in t
    if (error) {
      defer.reject(error);
    } else {
      ...
      volumioFormated.push({
        type: "title",
        title: "Play Music Tracks",
        availableListViews: ["list"],
        items: songList
      });
      volumioFormated.push({
        type: "title",
        title: "Play Music Artists",
        availableListViews: ["list", "grid"],
        items: artistList
      });
      volumioFormated.push({
        type: "title",
        title: "Play Music Albums",
        availableListViews: ["list", "grid"],
        items: albumsList
      });
      volumioFormated.push({
        type: "title",
        title: "Play Music Playlists",
        availableListViews: ["list"],
        items: playlist
      });
  ...
  service.playMusic.getArtist(
    ...
    function(error, response) {
      if (error) {
        ...
        var artistList = getArtistsFromList(response.related_artists, {
          isArtistList: true
        });
        var albumsList = getAlbumsFromList(response.albums, {
          isAlbumList: true
        });
        var topTrackList = getTracksFromList(service, response.topTracks, {
          isTrackArray: true
        });
...
playMusicController.prototype.previous = function() {
  ...
  return service.mpdPlugin.sendMpdCommand("previous", []).then(function() {
    return service.mpdPlugin.getState().then(function(state) {
      ...
      return service.commandRouter.stateMachine.syncState(
        state,
        "googleplaymusic"
      );
    });
