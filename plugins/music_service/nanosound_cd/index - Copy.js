		...
		if (!error && httpresponse.statusCode === 200) {
			...
			if(body["status"] == "NOTDONE")
			{
				...
				for (var i = 0; i < totalsongs; i++) {
					var trackkno = body["torip"][i];
					
					if(String(trackkno) in body)
					{
						found = true;
						var pct = body[String(trackkno)];
						if(pct!=1.0)
						{
							message = "Extracting Track " + String(trackkno) + " - " + Math.round(pct*10000) / 100 + "%. Total of " + String(totalsongs) + " tracks";
						}
