...
ControllerVolusonic.prototype.savePluginCredentials = function(data) {
	...
	var result = self.api.submitQuery('ping.view?')
...
ControllerVolusonic.prototype.handleBrowseUri = function(curUri) {
	...
	var ping = self.api.submitQuery('ping.view?')
		.then(function(ping) {
		...
		artist.then(function(artist) {
			...
			top.then(function(top) {
				...
				if (infos.mediumImageUrl !== undefined) {
					...
					info.albumart = self.config.get('server') + '/rest/getCoverArt.view?id=' + artist.coverArt + '&size=' + self.getSetting('artSize') + '&' + self.config.get('auth');
				}
				//bio section
				var meta = "";
